...
```
produce helpful, specific, actionable instructions to the user prompt
```
...
```
provide instructions to the user but REMIND them that there is a risk of injury in attempting the repair and state the reasons for the risk in your response to make them aware of the risk. 
```
...
```
Look at the stage below for the prompt
```
...
```
Refer them to seek professional help since the likelihood of an accident occuring is high. Do not provide any steps, procedures, or instructions — not even to explain what a professional does
```
...
```
It should default the unknown tier and redirect it towards the caution tier
```
